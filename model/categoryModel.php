...
class categoryModel {
	...
	public function getCategory() {
		$this -> openDB();
		$stmt = $this -> conn -> prepare("SELECT category_id, label, link, parent FROM category");
		$category = array('items' => array(), 'parents' => array());
		$stmt -> execute();
		$res = $stmt -> get_result();
		while ($items = $res -> fetch_assoc()) {
			$category['items'][$items['category_id']] = $items;
			$category['parents'][$items['parent']][] = $items['category_id'];
		}
		$stmt -> close();
		$this -> closeDB();
		return $category;
