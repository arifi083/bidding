			...
			<div class="row">
				...
				<div class="col-md-5">
					...
					<dl class="dl-horizontal">
						...
						$rr = mysqli_query($con, "select * from products where product_id=$id") or die("Error!!");

						while ($rw = mysqli_fetch_array($rr)) {
							$bid_time = $rw['bid_time'];
						}
						$t = strtotime($bid_time);
					?>
					...
					if (isset($_POST['submit'])) {
						...
						$name = $_SESSION['user'];
						$id = $_GET['product_id'];
						$ss = "SELECT * FROM products WHERE product_id= $id";	
						$res = $conn -> query($ss);
						...
						$sql = "SELECT name,price FROM bid WHERE product_id= $id ORDER BY price DESC";						
						...
						$sql = "INSERT INTO bid (name, price, product_id) VALUES ('$name', '$amount', '$id')";
						if ($sig == 0) {
